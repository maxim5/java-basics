...
import static io.spbx.util.base.error.BasicExceptions.*;
...
public final class Int128 extends Number implements Comparable<Int128> {
    ...
    public long longValueExact() {
        assert is64Bit() : "Int128 value is required to fit into 64-bit but it is out of range: " + this;
        return longValue();
    }

    public long longValueSaturated() {
        return is64Bit() ? longValue() : isPositive() ? Long.MAX_VALUE : Long.MIN_VALUE;
    }

    public boolean is32Bit() {
        return is64Bit() && intValue() == longValue();
    }

    public int intValueExact() {
        assert is32Bit() : "Int128 value is required to fit into 32-bit but it is out of range: " + this;
        return intValue();
    }

    public int intValueSaturated() {
        return is32Bit() ? intValue() : isPositive() ? Integer.MAX_VALUE : Integer.MIN_VALUE;
    }
    ...
    public @NotNull Int128 divide(long that) {
        return divide(Int128.from(that));
    }
