...
public class Reversibles {
    public static <A, B> @NotNull Reversible<A, B> fromNotNullFunctions(@NotNull Function<A, B> forward,
        ...
        LazyReversible<A, B> function = new LazyReversible<>() {
            @Override public @NotNull B forward(@NotNull A a) {
                return forward.apply(a);
            }
        };
        LazyReversible<B, A> reverse = new LazyReversible<>() {
            @Override public @NotNull A forward(@NotNull B b) {
    ...
    public static <A, B> @NotNull Reversible<A, B> fromNullableFunctions(@NotNull Function<A, B> forward,
        ...
        LazyReversible<A, B> function = new LazyReversible<>() {
            ...
            @Override public @Nullable B forwardNullable(@Nullable A a) {
                return forward.apply(a);
            }
        };
        LazyReversible<B, A> reverse = new LazyReversible<>() {
            ...
            @Override public @Nullable A forwardNullable(@Nullable B b) {
                return backward.apply(b);
            }
        };
        LazyReversible.bind(function, reverse);
    ...
    public static <U, V> @NotNull Reversible<List<U>, List<V>> extendToList(@NotNull Reversible<U, V> reversible) {
        return fromNotNullFunctions(list -> list.stream().map(reversible).toList(),
                                    list -> list.stream().map(reversible.reverse()).toList());
    }
}
