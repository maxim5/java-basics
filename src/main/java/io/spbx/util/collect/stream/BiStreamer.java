...
public final class BiStreamer<K, V> extends AbstractStreamer<Pair<K, V>, BiStreamer<K, V>> implements ToCollectKvApi<K, V, Pair<K, V>> {
    ...
    public <K2> @NotNull BiStreamer<K2, V> flatMapKeys(
            @NotNull Function<? super K, ? extends Stream<? extends K2>> keyMap) {
        return of(stream.flatMap(pair -> keyMap.apply(pair.first()).map(k -> Pair.of(k, pair.second()))));
    }
    ...
    public <V2> @NotNull BiStreamer<K, V2> flatMapValues(
        @NotNull Function<? super V, ? extends Stream<? extends V2>> valMap) {
        return of(stream.flatMap(pair -> valMap.apply(pair.second()).map(v -> Pair.of(pair.first(), v))));
    }
